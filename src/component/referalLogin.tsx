...
const ReferalLoginPage = () => {
  const { handleLogin, setIsLoading, handleGoogleAuth } = useContext(GameContext)!;
  const [email, setEmail] = useState("");
  const urlParams = new URLSearchParams(window.location.search);
  const referralCode = urlParams.get("referralCode");
  if (referralCode){
    localStorage.setItem("referCode", referralCode);}



 // const [password, setPassword] = useState(myConstants.default_pass);
//setIsLoading(true);
...
export default ReferalLoginPage;
